...
class Statements(indent: Int){
  ...
  def del_stmt[_: P] = P( kw("del") ~~ " ".rep ~~ exprlist ).map(x => Ast.stmt.Delete(x))
  ...
  def dotted_as_name[_: P]: P[Ast.alias] = P( dotted_name.map(x => Ast.identifier(x.map(_.name).mkString("."))) ~ (kw("as") ~ NAME).? ).map(x => Ast.alias(x._1, x._2))
  ...
  def global_stmt[_: P]: P[Ast.stmt.Global] = P( kw("global") ~ NAME.rep(sep = ",") ).map(x => Ast.stmt.Global(x))
  ...
  def assert_stmt[_: P]: P[Ast.stmt.Assert] = P( kw("assert") ~ test ~ ("," ~ test).? ).map(x => Ast.stmt.Assert(x._1, x._2))
  ...
  def while_stmt[_: P] = P( kw("while") ~/ test ~ ":" ~~ suite ~~ (space_indents ~~ kw("else") ~/ ":" ~~ suite).?.map(_.toSeq.flatten) ).map(x => Ast.stmt.While(x._1, x._2, x._3))
