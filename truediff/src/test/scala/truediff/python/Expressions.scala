...
object Expressions {
  ...
  def dictorsetmaker[_: P]: P[Ast.expr] = {
    ...
    def dict_comp = P(
      (dict_item ~ comp_for.rep(1)).map(x => Ast.expr.DictComp(x._1, x._2, x._3))
    )
    def set: P[Ast.expr.Set] = P( test.rep(1, ",") ~ ",".? ).map(x => Ast.expr.Set(x))
    def set_comp = P( test ~ comp_for.rep(1) ).map(x => Ast.expr.SetComp(x._1, x._2))
  ...
  def yield_expr[_: P]: P[Ast.expr.Yield] = P( kw("yield") ~ testlist.map(tuplize).? ).map(x => Ast.expr.Yield(x))
